...
class PatchL1Loss3D(nn.Module):
    def __init__(self, patch_size=4, stride=2, low_threshold=0.1, high_weight=5.0):
        super().__init__()
        self.patch_size = patch_size
        self.stride = stride
    ...
    def forward(self, fake, real):
        ...
        weight[real <= self.low_threshold] = self.high_weight
        ...
        if D_out <= 0 or H_out <= 0 or W_out <= 0:
            return torch.mean(weight * torch.abs(fake - real))

        # Reshape for unfold
        fake_reshaped = fake.view(B * C, 1, D, H, W)
        real_reshaped = real.view(B * C, 1, D, H, W)
        weight_reshaped = weight.view(B * C, 1, D, H, W)
        ...
        real_patches = real_reshaped.unfold(2, self.patch_size, self.stride) \
                                    .unfold(3, self.patch_size, self.stride) \
                                    .unfold(4, self.patch_size, self.stride)
        weight_patches = weight_reshaped.unfold(2, self.patch_size, self.stride) \
                                      .unfold(3, self.patch_size, self.stride) \
                                      .unfold(4, self.patch_size, self.stride)
        ...
        fake_patches = fake_patches.contiguous().view(B * C, ps * ps * ps, -1)
        real_patches = real_patches.contiguous().view(B * C, ps * ps * ps, -1)
        weight_patches = weight_patches.contiguous().view(B * C, ps * ps * ps, -1)

        # 加权 L1 per patch
        l1_per_patch = torch.mean(weight_patches * torch.abs(fake_patches - real_patches), dim=1)  # [B*C, N]
        loss = torch.mean(l1_per_patch)
        return loss

import torch

def average_tensor_grid(tensor_grid):
    """
    对一个二维列表（每个元素是 torch.Tensor）中的每个 tensor 求全局平均值，
    返回一个形状相同的 torch.Tensor，其中每个元素是对应 tensor 的平均值。

    参数:
        tensor_grid (list of list of torch.Tensor): 二维列表，每个元素是一个 tensor

    返回:
        torch.Tensor: 与输入列表形状一致的 tensor，dtype 通常为 float
    """
    if not tensor_grid:
        raise ValueError("Input tensor_grid is empty.")
    
    rows = len(tensor_grid)
    cols = len(tensor_grid[0])
    
    if cols == 0:
        raise ValueError("Each row in tensor_grid must contain at least one tensor.")
    
    # 可选：检查是否为矩形（所有行长度一致）
    for i, row in enumerate(tensor_grid):
        if len(row) != cols:
            raise ValueError(f"Row {i} has {len(row)} columns, but expected {cols} (non-rectangular input).")

    # 遍历所有位置，计算每个 tensor 的全局平均值
    avg_rows = []
    for i in range(rows):
        avg_row = []
        for j in range(cols):
            mean_val = tensor_grid[i][j].mean()
            avg_row.append(mean_val)
        avg_rows.append(torch.stack(avg_row))
    
    result = torch.stack(avg_rows)
    return result
...
def main_train(train_dataloader, val_dataloader, epochs, device, resume=True, only_test=False, save_nii=False,
    ...
    pl1_loss_fn = PatchL1Loss3D(patch_size=4, stride=2)
    ...
    for epoch in range(epochs):
        ...
        if not only_test:
            ...
            for (MRI, PET, name) in train_loader_with_progress:
                ...
                b_input = PET#mappingPET
                input_data = MRI#mappingPET#+pre_result #新方法
                #b_pre = mappingPET#+pre_result
                GT = PET  #新方法
                if mohu:
                    with torch.no_grad():
                ...
                out, x_recon,mapping,pre,_= model(input_data, b_input)
                ...
                loss =  pl1_loss_fn(out, GT)*5 +F.l1_loss(out, GT)*5#+
                loss += F.l1_loss(x_recon, input_data)*2
                loss += F.l1_loss(mapping, b_input)
                loss += F.l1_loss(pre, b_input)
        ...
        if epoch % 1 == 0:
            ...
            with torch.no_grad():
                for (MRI, PET, name) in val_loader_with_progress:
                    ...
                    out,x_recon,mapping,pre,A = model(input_data, b_input)
                    #A_all += average_tensor_grid(A).cpu()
                    #out = model(MRI)
                    ...
                    if save_nii:
                        save_tensor_to_nii(out, name[0],
                                           dir='')
                        save_tensor_to_nii(x_recon, name[0],
                                           dir='')
                        save_tensor_to_nii(mapping, name[0],
                                           dir='')
                        save_tensor_to_nii(pre, name[0],
                                           dir='')
                #np.save('A_all.npy',A_all.cpu().detach().numpy())
                if not save_nii:
